...
# معايرة على 100 نص منافس حقيقي: العتبة ≥3 ظهورات (لا 2) لتفادي رفض نصوص
# بشرية أصيلة تعيد كلمة أساسية مرتين بلا قصد سرد ملتصق (كانت 18/100 زوراً).
def test_detect_nonadjacent_word_repeat():
    tells = hz.detect(
        'المقدمه فريشه والقلب زهري المقدمه حاره والقلب دافي المقدمه غنيه وهادية'
    )
...
def test_repeat_detection_is_content_tell():
    ct = hz.content_tells(
        'ثباته على الجلد أقل شوي ثباته يختلف حسب البشره بس بشكل عام ثباته ممتاز صراحه'
    )
    assert 'تكرار كلمة محورية' in ct


def test_two_repeats_not_flagged():
    # ظهور مرتين فقط (لا 3+) بشري طبيعي — لا يُحسب بعد المعايرة
    assert hz.detect(
...
def test_adjacent_repeat_not_flagged():
    # «فخم فخم فخم» تشديد لهجة مقصود ومعاير على بيانات منافسين حقيقية —
    # لا يُحسب مهما تكرر طالما كل الظهورات متجاورة محضاً
    assert hz.detect('فخم فخم فخم بمعنى الكلمة والله صراحه اكيد') == []
...
# ═══════════ قاعدة الأربعة (رُفعت من ثلاثة بعد معايرة 14%→1% خطأ) ═══════════
def test_three_conjuncts_not_flagged():
    # عطف ثلاثي (صفتان مربوطتان بـ«و» + الأصل = 3 عناصر) عادي جداً بالعامية
    # السعودية — لم يعد يُحسب بصمة بوت بعد رفع العتبة لأربعة
    assert hz.detect('عطر ثابت وفواح وجميل جدا صراحه') == []


def test_four_conjuncts_still_flagged():
    tells = hz.detect('عطر راقي ونظيف وهادي ودافي جدا صراحه')
    assert 'قاعدة الثلاثة' in tells
